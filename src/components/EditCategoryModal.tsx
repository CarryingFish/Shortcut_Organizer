...
export default function EditCategoryModal({ category, onClose }: EditCategoryModalProps) {
  const { updateCategory, deleteCategory, categories } = useShortcutContext();
  ...
  useEffect(() => {
    if (category) {
      ...
      // 识别是否为自定义颜色（hex/rgb）
      const isCustom = /^#|^rgb\(/i.test(category.color || '');
      if (isCustom) {
        setUseCustom(true);
        setCustomColor(category.color as unknown as string);
        // 保留一个预设默认，避免按钮高亮
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">编辑分类</h2>
            <button 
              onClick={onClose}
